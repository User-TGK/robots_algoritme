...
{
    cv::Mat originalCopy, denoised, bilateralFiltered, hsvFiltered, threshFiltered;
    originalCopy = original;

    // Remove all (color-) noise from the images
    cv::fastNlMeansDenoisingColored(originalCopy, denoised, 5, 5, 5, 11);
...
std::map<supportedColor, colorRange> Extractor::mappedColorRangesMapFromFile() const
{
    ...
    std::ifstream calibrationFile("../calibration/calibration.ini");
...
{
    Parser parser;
    std::ofstream calibrationFile("../calibration/calibration.ini");
