...
private:
    ...
    /** 0.15 matches a 0.85 resemblances beteween a reference shape and detected shape to be recognized as a supported shape */
    double minResultFactor = 0.15;

    /** Map that contains the set of contours for the reference shapes */
    std::unordered_map<supportedShape, std::vector<cv::Point>> referenceContours;
